...
class User(db.Model):
    ...
    role = db.Column(db.String(20), nullable=False, default='traveler')  # traveler, owner, admin
    ...
    # Relationships
    owner_profile = db.relationship('Owner', backref='user', uselist=False, cascade='all, delete-orphan')
...
class Owner(db.Model):
    ...
    kyc_status = db.Column(db.String(20), nullable=False, default='verified')  # pending, verified, rejected
    ...
    complaints = db.relationship('Complaint', backref='owner', lazy='dynamic')
...
class Admin(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('users.user_id', ondelete='CASCADE'), unique=True, nullable=False)
    admin_level = db.Column(db.String(50), nullable=False, default='Super Admin')
    department = db.Column(db.String(100), nullable=False, default='Operations')
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class Property(db.Model):
    ...
    property_type = db.Column(db.String(80), nullable=False)  # Heritage Villa, Traditional Tharavadu, Nalukettu, Colonial Manor
    description = db.Column(db.Text, nullable=False)
    address = db.Column(db.String(255), nullable=False)
    district = db.Column(db.String(50), nullable=False)  # Ernakulam, Thrissur
    latitude = db.Column(db.Float, nullable=False)
    longitude = db.Column(db.Float, nullable=False)
    price_per_night = db.Column(db.Float, nullable=False)
    guest_capacity = db.Column(db.Integer, nullable=False, default=2)
    bedrooms = db.Column(db.Integer, nullable=False, default=1)
    bathrooms = db.Column(db.Integer, nullable=False, default=1)
    amenities = db.Column(db.Text, nullable=True)  # JSON serialized list
    images = db.Column(db.Text, nullable=True)  # JSON serialized list of image paths/URLs
    featured_image = db.Column(db.String(255), nullable=True)
    approval_status = db.Column(db.String(20), nullable=False, default='pending')  # pending, approved, rejected
    rejection_reason = db.Column(db.Text, nullable=True)
    availability_status = db.Column(db.String(20), nullable=False, default='available')  # available, booked, maintenance
    ...
    def images_list(self):
        if not self.images:
            return [self.featured_image] if self.featured_image else []
        try:
    ...
    def to_dict(self):
        return {
            ...
            'owner_id': self.owner_id,
            'owner_name': self.owner.user.full_name if self.owner and self.owner.user else 'UniRent Host',
...
class Vehicle(db.Model):
    ...
    owner_id = db.Column(db.Integer, db.ForeignKey('owners.owner_id', ondelete='CASCADE'), nullable=False)
    vehicle_name = db.Column(db.String(160), nullable=False)
    model = db.Column(db.String(100), nullable=False)
    vehicle_type = db.Column(db.String(20), nullable=False)  # car, bike, scooter
    description = db.Column(db.Text, nullable=False)
    ...
    specifications = db.Column(db.Text, nullable=True)  # JSON or key:value
    images = db.Column(db.Text, nullable=True)
    featured_image = db.Column(db.String(255), nullable=True)
    approval_status = db.Column(db.String(20), nullable=False, default='pending')  # pending, approved, rejected
    rejection_reason = db.Column(db.Text, nullable=True)
    availability_status = db.Column(db.String(20), nullable=False, default='available')  # available, rented, maintenance
    featured = db.Column(db.Boolean, nullable=False, default=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class Booking(db.Model):
    ...
    owner_id = db.Column(db.Integer, db.ForeignKey('owners.owner_id', ondelete='CASCADE'), nullable=False)
    listing_type = db.Column(db.String(20), nullable=False)  # property, vehicle
    property_id = db.Column(db.Integer, db.ForeignKey('properties.property_id', ondelete='SET NULL'), nullable=True)
    vehicle_id = db.Column(db.Integer, db.ForeignKey('vehicles.vehicle_id', ondelete='SET NULL'), nullable=True)
    ...
    booking_status = db.Column(db.String(20), nullable=False, default='confirmed')  # pending, confirmed, active, completed, cancelled
    ...
    complaints = db.relationship('Complaint', backref='booking', lazy='dynamic', cascade='all, delete-orphan')
    ...
    def listing_name(self):
        if self.listing_type == 'property' and self.property:
            ...
            return f"{self.vehicle.vehicle_name} ({self.vehicle.model})"
        return 'Listing'
    ...
    def to_dict(self):
        return {
            ...
            'user_id': self.user_id,
            'traveler_name': self.user.full_name if self.user else None,
            'owner_id': self.owner_id,
            ...
            'payment_status': self.payment.payment_status if self.payment else 'pending',
...
class Payment(db.Model):
    ...
    booking_id = db.Column(db.Integer, db.ForeignKey('bookings.booking_id', ondelete='CASCADE'), unique=True, nullable=False)
    amount = db.Column(db.Float, nullable=False)
    payment_method = db.Column(db.String(50), nullable=False, default='card')  # card, upi, netbanking
    transaction_reference = db.Column(db.String(100), unique=True, nullable=False)
    payment_status = db.Column(db.String(20), nullable=False, default='successful')  # pending, successful, failed, refunded
...
class Review(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('users.user_id', ondelete='CASCADE'), nullable=False)
    listing_type = db.Column(db.String(20), nullable=False)  # property, vehicle
    property_id = db.Column(db.Integer, db.ForeignKey('properties.property_id', ondelete='CASCADE'), nullable=True)
    vehicle_id = db.Column(db.Integer, db.ForeignKey('vehicles.vehicle_id', ondelete='CASCADE'), nullable=True)
    rating = db.Column(db.Integer, nullable=False)  # 1 to 5
    review_title = db.Column(db.String(150), nullable=True)
    ...
    def to_dict(self):
        return {
            ...
            'booking_id': self.booking_id,
            'author_name': self.user.full_name if self.user else 'Guest Traveler',
            'listing_type': self.listing_type,
...
class Complaint(db.Model):
    ...
    complaint_reference = db.Column(db.String(50), unique=True, nullable=False)
    booking_id = db.Column(db.Integer, db.ForeignKey('bookings.booking_id', ondelete='CASCADE'), nullable=False)
    user_id = db.Column(db.Integer, db.ForeignKey('users.user_id', ondelete='CASCADE'), nullable=False)
    owner_id = db.Column(db.Integer, db.ForeignKey('owners.owner_id', ondelete='CASCADE'), nullable=False)
    category = db.Column(db.String(60), nullable=False)
    # Categories: Property Damage, Vehicle Issue, Cleanliness, Missing Facility, Owner/Service Issue, Payment Issue, Safety Concern, Other
    description = db.Column(db.Text, nullable=False)
    evidence_image = db.Column(db.String(255), nullable=True)
    status = db.Column(db.String(20), nullable=False, default='open')
    # Statuses: open, under_review, owner_responded, resolved, rejected
    ...
    def to_dict(self):
        return {
            ...
            'booking_reference': self.booking.booking_reference if self.booking else None,
            'traveler_name': self.user.full_name if self.user else None,
            'owner_name': self.owner.user.full_name if self.owner and self.owner.user else None,
